...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $con->prepare("SELECT faculty FROM tbl_service_requests WHERE id = ?");
    ...
    $stmt = $con->prepare("UPDATE tbl_service_requests SET current_stage = ? WHERE id = ?");
    $stmt->bind_param("si", $new_stage, $request_id);
    
    if ($stmt->execute()) {
        // Redirect to certification_request.php with a success message
        header("Location: service_request.php?updated=success");
        exit();
    } else {
        // Redirect to certification_request.php with an error message
        header("Location: service_request.php?updated=error");
        exit();
    }
} else {
